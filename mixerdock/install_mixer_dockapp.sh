...
if [ "$(id -u)" -ne 0 ]; then
    echo "Run me as root:  sudo $(pwd)/install_mixer_dockapp.sh" >&2
    exit 1
fi
...
install -o root -g wheel -m 755 "$SRC" /usr/local/bin/mixer_dockapp
echo "Installed: /usr/local/bin/mixer_dockapp (from $SRC)"